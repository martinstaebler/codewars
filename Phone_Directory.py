...
regex_name = r"\<(.*?)\>"
regex_number = r"[+][0-9]{1,3}[\s-]\d{3}[(\s)?-]\d{3}[\s-]\d{4}"
phone_book = list()
...
def phone(strng, num):
    addresses = strng.split('\n')

    for line in addresses:
        print(line)
        eintrag = list()
        eintrag.append(re.findall(regex_name, line))
        eintrag.append(re.findall(regex_number, line))
        print(eintrag)
        #phone_book.append(eintrag)
    #print(addresses)
    #print(strng.index(num))
